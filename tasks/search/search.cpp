...
#include <iostream>
//тут я сравниваю строки и хочу чтобы мой вектор
bool CompareMeaning(std::vector<double>& meaning1, std::vector<double>& meaning2) {
    return meaning1[0] < meaning2[0];
}
bool Compare(std::vector<double>& meaning1, std::vector<double>& meaning2) {
...
std::vector<std::string_view> Search(std::string_view text, std::string_view query, size_t results_count) {
    ...
    for (size_t j = 0; j < text.size(); ++j) {
        if (text[j] == '\n' || j == text.size() - 1) {
            if (j + 1 == text.size()) {
                ...
                if (!slice.empty()) {
                    quantity_rows += 1;
                    SearchWords(slice, repository, query_words, begin, j + 1);
                    begin = j + 1;
                }
            } else {
                slice = static_cast<std::string>(text.substr(begin, j - begin));
                if (!slice.empty()) {
                    SearchWords(slice, repository, query_words, begin, j);
                    begin = j + 1;
                    quantity_rows += 1;
                }
            }
    ...
    //
    std::sort(relevant.begin(), relevant.end(), CompareMeaning);
    ...
    std::sort(pred_answer.begin(), pred_answer.end(), Compare);
